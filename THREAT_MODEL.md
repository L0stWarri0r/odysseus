...
1. **`/api/v1/chat` `base_url` SSRF.** Chat-scoped API tokens can no longer forward LLM requests to arbitrary private/internal hosts; `routes/webhook_routes.py` validates `base_url` with `validate_public_http_url`, and `tests/test_api_chat_security.py` covers the regression.
