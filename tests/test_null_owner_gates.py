...
def test_gallery_owner_filter_allows_single_user_mode():
    ...
    # user=None is the documented single-user/auth-disabled mode for gallery.
    fake_q.filter.assert_not_called()
    assert out is fake_q


def test_gallery_owner_filter_passes_user(monkeypatch):
    from routes import gallery_helpers
    from routes.gallery_routes import _owner_filter

    monkeypatch.setattr(
        gallery_helpers,
        "GalleryImage",
        SimpleNamespace(owner=MagicMock()),
    )
    fake_q = MagicMock()
