...
from src.auth_helpers import get_current_user, require_privilege, owner_filter
...
def _image_endpoint_query(db, request: Request, user: str):
...
def setup_gallery_routes() -> APIRouter:
    ...
    async def gallery_ai_upscale(request: Request):
        ...
        db = SessionLocal()
        try:
            ep = _image_endpoint_query(db, request, user).first()
        finally:
            db.close()
    ...
    async def inpaint_proxy(request: Request):
        ...
        if not base:
            db = SessionLocal()
            try:
                eps = _image_endpoint_query(db, request, user).all()
                if not eps:
            ...
            db = SessionLocal()
            try:
                for ep in _image_endpoint_query(db, request, user).all():
