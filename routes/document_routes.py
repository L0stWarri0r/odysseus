    ...
    async def create_document(request: Request, req: DocumentCreate) -> Dict[str, Any]:
        ...
        try:
            ...
            if req.session_id:
                ...
                # Match document ownership elsewhere: authenticated callers
                # may not write into a null-owner or foreign session.
                if user and (not session.owner or session.owner != user):
    ...
    async def import_pdf(
        ...
        if session_id:
            ...
            try:
                ...
                if not sess:
                    raise HTTPException(404, "Session not found")
                if user and (not sess.owner or sess.owner != user):
