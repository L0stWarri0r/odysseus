...
def _owned_enabled_endpoint(db, endpoint_id: Optional[str], user: str):
    """Load an enabled ModelEndpoint scoped to ``user`` (plus shared null-owner)."""
...
def setup_research_routes(research_handler, session_manager=None) -> APIRouter:
    ...
    async def research_start(body: ResearchStartRequest, request: Request):
        ...
        if body.endpoint_id:
            ...
            db = SessionLocal()
            try:
                ep = _owned_enabled_endpoint(db, body.endpoint_id, user)
                if not ep:
            ...
            if not ep_url:
                ...
                db = SessionLocal()
                try:
                    ep = _owned_enabled_endpoint(db, None, user)
                    if ep:
                        base = normalize_base(ep.base_url)
                        ep_url = build_chat_url(base)
                        ep_headers = build_headers(ep.api_key, base)
    ...
    async def research_spinoff(session_id: str, request: Request):
        ...
        if not ep_url or not ep_model:
            # Last resort: any enabled endpoint owned by (or shared with) the user
            from src.database import SessionLocal
            from src.endpoint_resolver import normalize_base, build_chat_url, build_headers
            db = SessionLocal()
            try:
